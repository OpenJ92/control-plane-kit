...
@dataclass(frozen=True)
class SelectDesiredTopologyDraft:
    context: TrustedCommandContext
    session_id: str
    draft_id: str
    revision: int
...
@dataclass(frozen=True)
class DeleteDesiredTopologyDraft:
    context: TrustedCommandContext
    session_id: str
    draft_id: str
    expected_head_revision: int
...
@dataclass(frozen=True)
class DesiredTopologyDraftSelectionResult(DesiredTopologyDraftResult):
    desired_realized_projection_id: str
    desired_graph_revision: int

    def __post_init__(self) -> None:
        super().__post_init__()
        _text(self.desired_realized_projection_id, "desired_realized_projection_id")
        _revision_number(self.desired_graph_revision)


@dataclass(frozen=True)
class DesiredTopologyDraftDeletionResult:
    workspace_id: str
    draft_id: str
    head_revision: int
    deleted_by: str
    ...
    def __post_init__(self) -> None:
        for name in ("workspace_id", "draft_id", "deleted_by"):
            _text(getattr(self, name), name)
        _revision_number(self.head_revision)
        validate_canonical_utc_timestamp(self.deleted_at)
...
class DesiredTopologyDraftCommandService:
    ...
    def execute(self, command: CreateDesiredTopologyDraft | ReviseDesiredTopologyDraft | SelectDesiredTopologyDraft | DeleteDesiredTopologyDraft) -> DesiredTopologyDraftResult | DesiredTopologyDraftDeletionResult:
        if type(command) in (SelectDesiredTopologyDraft, DeleteDesiredTopologyDraft):
            return _execute_reference_command(self, command)
...
def _reference_command_intent(command):
    context = command.context
    if (not isinstance(context, TrustedCommandContext)
            or PolicyScope.INSTANCE_WORKSPACE_EDIT not in context.granted_scopes):
        raise DesiredTopologyDraftError("workspace edit authority is required")
    for name, value in (("workspace_id", context.workspace_id), ("actor_id", context.actor_id),
                        ("session_id", command.session_id), ("draft_id", command.draft_id)):
        _text(value, name)
    if not isinstance(command.idempotency_key, IdempotencyKey):
        raise DesiredTopologyDraftError("idempotency key is required")
    selecting = type(command) is SelectDesiredTopologyDraft
    kind = (OperatorCommandKind.SELECT_DESIRED_TOPOLOGY_DRAFT if selecting
            else OperatorCommandKind.DELETE_DESIRED_TOPOLOGY_DRAFT)
    intent = {"kind": kind.value, "context": context.descriptor(),
              "session_id": command.session_id, "draft_id": command.draft_id}
    if selecting:
        _revision_number(command.revision)
        for name in ("expected_desired_graph_id", "expected_desired_realized_projection_id"):
            value = getattr(command, name)
            if value is not None:
                _text(value, name)
            intent[name] = value
        if ((command.expected_desired_graph_id is None)
                != (command.expected_desired_realized_projection_id is None)):
            raise DesiredTopologyDraftError("expected desired lineage must be paired")
        generation = command.expected_desired_graph_revision
    ...
    fingerprint = hashlib.sha256(json.dumps(intent, sort_keys=True, separators=(",", ":"),
        ensure_ascii=False, allow_nan=False).encode()).hexdigest()
    return kind, fingerprint


def _execute_reference_command(service, command):
    kind, fingerprint = _reference_command_intent(command)
    context = command.context
    selecting = type(command) is SelectDesiredTopologyDraft
    with service._uow() as uow:
        ...
        try:
            ...
            draft = uow.stores.desired_topology_drafts.get(context.workspace_id, command.draft_id, for_update=True)
        except KeyError:
            raise DesiredTopologyDraftError("workspace draft was not found") from None
        if draft.deleted_at is not None:
            raise DesiredTopologyDraftConflict("draft is retired")
        if selecting:
            # Existing authoring CAS: hold workspace lock, compare the complete
            # expected tuple, then update/increment in the same transaction.
            if ((workspace.desired_graph_id, workspace.desired_realized_projection_id,
                ...
                raise DesiredTopologyDraftConflict("desired lineage is stale")
            try:
                revision = uow.stores.desired_topology_drafts.revision(
                    context.workspace_id, command.draft_id, command.revision)
                graph = uow.stores.graphs.get(revision.graph_id)
                if graph.workspace_id != context.workspace_id:
                    raise ValueError
                topology = DEFAULT_GRAPH_CODEC.decode(graph.graph_descriptor)
                validate_graph(topology).require_valid()
                for reference in product_references_in_graph(topology):
                    registered = uow.stores.registered_products.get(context.workspace_id, reference)
                    if registered.status is not RegisteredProductStatus.ACTIVE:
                        raise ValueError
            ...
            if draft.head_revision != command.expected_head_revision:
                raise DesiredTopologyDraftConflict("draft head is stale")
            if uow.stores.desired_topology_drafts.has_references(context.workspace_id, command.draft_id):
                raise DesiredTopologyDraftConflict("draft history is still referenced")
        action_id = service._id()
...
def _validate_identity_projection(projection, graph):
    ...
    if projection != expected:
        raise ValueError("identity projection is incongruent")


def _reference_replay(uow, command, action):
    """Validate retained evidence, never re-admit against mutable desired truth."""
    try:
        if action.actor_id != command.context.actor_id or action.session_id != command.session_id:
            raise ValueError
        session = uow.stores.activity_history.get_session(command.session_id)
        if session.workspace_id != command.context.workspace_id:
            raise ValueError
        ...
        if type(command) is SelectDesiredTopologyDraft:
            ...
            revision = uow.stores.desired_topology_drafts.revision(
                result.workspace_id, result.draft_id, result.revision)
            graph = uow.stores.graphs.get(result.graph_id)
            if revision.graph_id != result.graph_id or graph.workspace_id != result.workspace_id:
                raise ValueError
            projection = uow.stores.realized_graphs.get(result.desired_realized_projection_id)
            _validate_identity_projection(projection, graph)
        else:
            if set(payload) != {"workspace_id", "draft_id", "head_revision", "deleted_by", "deleted_at"}:
                raise ValueError
            result = DesiredTopologyDraftDeletionResult(**payload)
            draft = uow.stores.desired_topology_drafts.get(result.workspace_id, result.draft_id)
            if ((result.head_revision, result.deleted_by, result.deleted_at)
                    != (draft.head_revision, draft.deleted_by, draft.deleted_at)
                    or result.head_revision != command.expected_head_revision
                    or result.deleted_by != action.actor_id or result.deleted_at != action.created_at):
                raise ValueError
        if (result.workspace_id != command.context.workspace_id or result.draft_id != command.draft_id):
            raise ValueError
        return result
    except (KeyError, ValueError, TypeError, AttributeError):
        raise DesiredTopologyDraftConflict("draft replay evidence is malformed or incongruent") from None
