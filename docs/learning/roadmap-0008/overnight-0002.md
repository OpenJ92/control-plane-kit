...
## #529 Postgres-Backed Service DeploymentProgram Acceptance

### Result

The heterogeneous service recipe now traverses the canonical persisted
deployment lifecycle without a second runner or production abstraction:

```text
DeploymentProgram.between(empty, service_graph)
  -> plan
  -> reconstruct from plan_id on a fresh program composition
  -> approve
  -> reconstruct from plan_id on another fresh composition
  -> admit -> claim -> execute -> advance
```

The test composition uses the same public services as the application:
...
DeploymentProgramServices(
    ...
    contexts=DeploymentPlanContextQueryService(...),
)
```

### Executable Laws

Real-Postgres acceptance now proves:

- empty-to-service deployment advances current graph truth only after terminal
  successful execution;
- plan and exact graph material reconstruct across independent program/service
  compositions;
- every materialized effect remains pinned to the persisted desired graph;
- the existing typed synthetic capability provider executes outside every
  UnitOfWork transaction;
- health observations persist separately and project through
  `InstanceReadService` without rewriting graph truth;
- package-owned semantic verification contracts materialize from the exact
  graph-pinned `StartNode` material, execute through the canonical
  `VerificationCommandService`, and publish separate intent/result
  observations;
- redacted graph projections retain package-product and Postgres protocol
  identity while omitting environment addresses and opaque secret references;
- an invalid desired graph may be recorded as operator edit intent, but records
  no plan, approval, execution request, run, effect, or current-graph
  advancement;
- a no-change transition records a durable empty plan but requests no approval
  and performs no runtime effect;
- teardown reconstructs from durable plan identity, executes typed removal
  effects, and advances to the exact empty graph.

### Breaking Point And Correction

The first run reused `service-deploy:approval` for both the approval-request
action created by `Plan` and the later manager decision. Production correctly
rejected one idempotency identity naming two operator commands. The acceptance
fixture now uses the distinct `service-deploy:decision` identity. No production
law or assertion changed.

The explicit verification proof then exposed a real contract mismatch:
`VerificationIdentity` applied the author-facing slug grammar to a durable
`graph_id`, while canonical graph versions generated by application services
may contain `:`. The model now distinguishes those identities:

```text
node_id, check_id = strict authored slugs
graph_id          = bounded nonblank control-free durable identity
```

This accepts canonical reconstructed graph identities without weakening node
or check vocabulary. Empty, blank, control-character-bearing, and oversized
graph identities still fail at construction.

```text
focused verification and real-Postgres tests:               15 passed
complete Docker/Postgres suite:                            1032 passed
assertions weakened:                                             0
skips or mocks added:                                            0
transactions spanning typed effects:                            0
parallel workflow or projection models:                         0
```

### Handoff To #532 And #530

- persisted planning, approval, execution, observation, no-op, and teardown
  semantics are proven for the exact #528 graph;
- retain this same `DeploymentProgram` composition for the live proof;
- #532 must interpret canonical `postgresql+psycopg://` graph URLs only at the
  service-discovery psycopg driver boundary;
- do not change the topology URL, Postgres socket protocol, or environment
  assignment to satisfy the driver;
