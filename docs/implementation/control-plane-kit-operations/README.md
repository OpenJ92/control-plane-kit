...
The [initial inventory](inventory.json) covers 310 tracked implementation, test, schema and meaningful process/build inputs: twenty-two reviewed companions, 285 pending entries and three exclusions. Existing guidance is maintained directly rather than recursively mirrored. The reviewed notes cover attempt-intent admission/history, runtime translation/bootstrap, execution/rotation-child admission, approval requests/decisions and read-service foundations; the PR records independent review depth. Pending means rollout work, not certified absence of defects or a reason to block unrelated work.
