"""Pure strict rendering from typed configuration values to artifacts."""
...
class ConfigurationParameters(Protocol):
    """A product exposes only values intended for its configuration template."""

    def configuration_values(self) -> Mapping[str, ConfigurationValue]: ...


class ConfigurationRenderingError(ValueError):
    """Typed configuration could not become a safe artifact."""
...
def _normalize_value(value: object, *, path: str) -> ConfigurationValue:
    ...
    if isinstance(value, tuple):
        return tuple(_normalize_value(item, path=f"{path}[]") for item in value)
