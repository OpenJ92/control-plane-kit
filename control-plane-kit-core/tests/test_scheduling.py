...
class SchedulingSuccessorTests(unittest.TestCase):
    ...
    def test_blocking_propagates_transitively(self) -> None:
        plan = ActivityPlan(
            (
                activity("first"),
    ...
    def test_cancellation_blocks_pending_but_preserves_running_evidence(self) -> None:
        schedule = derive_schedule(
            self.plan,
            evidence_for(
                self.plan,
    ...
    def test_same_plan_and_evidence_reconstruct_the_same_schedule(self) -> None:
    ...
    def test_compensation_waits_for_in_flight_forward_and_compensation(self) -> None:
    ...
    def test_missing_foreign_and_duplicate_evidence_fail_closed(self) -> None:
    ...
    def test_incoherent_completion_and_failure_evidence_fail_closed(self) -> None:
        complete = evidence_for(self.plan)
        with self.assertRaises(ScheduleEvidenceError):
            derive_schedule(
                self.plan,
