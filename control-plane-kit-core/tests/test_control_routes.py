...
class ControlRouteTests(unittest.TestCase):
    def test_common_status_routes_match_protocol_contract(self) -> None:
        routes = {
    ...
    def test_all_required_route_sets_are_present(self) -> None:
        descriptors = {
            route_set.name: {
        ...
        self.assertEqual(
            set(descriptors),
            {
                ...
                ControlRouteSetName.NODE_HEALTH,
            },
        )
        self.assertEqual(
    ...
    def test_route_set_named_accepts_string_or_enum(self) -> None:
        self.assertIs(
            route_set_named("targets"),
            route_set_named(ControlRouteSetName.TARGETS),
        )

    def test_unknown_route_set_fails_loudly(self) -> None:
        with self.assertRaises(KeyError):
            route_set_named("nope")

    def test_route_sets_have_json_friendly_descriptors(self) -> None:
    ...
    def test_control_path_uses_configurable_prefix(self) -> None:
