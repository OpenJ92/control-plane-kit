...
class AuthorizationHistoryParityTests(unittest.TestCase):
    def test_security_parity_covers_read_and_command_operations(self) -> None:
        ...
        self.assertEqual(len(parity.operations), 64)
        ...
        for operation_id in ("read.session-actions", "read.run-events"):
            journal = parity.operation(operation_id)
            self.assertEqual(journal.auth_scope, HttpAuthScope.READ)
            self.assertEqual(journal.safety, HttpOperationSafety.READ_ONLY)
            self.assertEqual(
                journal.activity_history,
                ActivityHistoryPolicy.NOT_RECORDED,
            )

        approval_detail = parity.operation("read.approval-detail")
        self.assertEqual(approval_detail.auth_scope, HttpAuthScope.READ)
        self.assertEqual(approval_detail.safety, HttpOperationSafety.READ_ONLY)
        self.assertEqual(
            approval_detail.activity_history,
            ActivityHistoryPolicy.NOT_RECORDED,
        )
        ...
        approval_request = parity.operation("approval.request")
        self.assertEqual(approval_request.auth_scope, HttpAuthScope.PLAN_WRITE)
        self.assertEqual(approval_request.safety, HttpOperationSafety.COMMAND)
        self.assertEqual(
            approval_request.activity_history,
            ActivityHistoryPolicy.RECORD_ACCEPTED_AND_REJECTED_COMMANDS,
        )

        preparation = parity.operation("deployment.prepare")
        self.assertEqual(preparation.auth_scope, HttpAuthScope.PLAN_WRITE)
        self.assertEqual(preparation.safety, HttpOperationSafety.COMMAND)
        self.assertEqual(
            preparation.activity_history,
            ActivityHistoryPolicy.RECORD_ACCEPTED_AND_REJECTED_COMMANDS,
        )

        advancement = parity.operation("graph.advance-current")
        self.assertEqual(advancement.auth_scope, HttpAuthScope.EXECUTION_RUN)
        self.assertEqual(advancement.safety, HttpOperationSafety.COMMAND)
        self.assertEqual(
            advancement.activity_history,
            ActivityHistoryPolicy.RECORD_ACCEPTED_AND_REJECTED_COMMANDS,
        )

        run_start = parity.operation("run.start")
        self.assertEqual(run_start.auth_scope, HttpAuthScope.EXECUTION_RUN)
        self.assertEqual(run_start.safety, HttpOperationSafety.COMMAND)
        self.assertEqual(
