...
class ProbeIntentTests(unittest.TestCase):
    def test_process_transport_health_and_readiness_are_distinct_objects(self) -> None:
        ...
        process = process_probe(node, "graph-a", policy)
        transport = transport_probe(node, endpoint, policy)
        health = application_health_probe(node, endpoint, policy)
        readiness = ReadinessProbeIntent(node.node_id, "graph-a")
    ...
    def test_application_health_is_derived_only_from_matching_http_material(self) -> None:
        intent = application_health_probe(_node(), _endpoint(), ProbePolicy())
...
def _node(*, health_path: str | None = "/health") -> NodeMaterial:
    return NodeMaterial(
        "api",
        _runtime(),
        ...
        (
            EndpointMaterial(
                ...
                LiteralEndpointMaterial("http://docker-api:8000"),
            ),
        ),
        (),
        health_path,
